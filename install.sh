...
# pyyaml 门槛：opencode / codex / zcode / dsh / grok / openclaw 的 agent/skill 转换依赖 pyyaml（与
# tools/platforms.py ensure_yaml 的运行时规则对齐）；claude 平台纯复制不转换、
# hermes/deepseek-tui 未走转换，均不需要。缺失时同样在任何目录创建/
# 删除前 fail-fast，而不是等 init.py / sync-project.py 执行时才报错。
case "$PLATFORM" in
    opencode|codex|zcode|dsh|grok|openclaw)
