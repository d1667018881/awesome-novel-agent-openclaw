...
支持平台（目录约定在 PLATFORMS 注册表声明，一行一条）：
  ...
  - openclaw → .openclaw/ agents=None（agents 即 skills，OpenClaw 无项目级 agents 目录，
                          子代理即 skill，由 sessions_spawn 调度）, skills=skills,
                          knowledge, memory
...
_DISPATCH_SECTIONS = {
    ...
    "openclaw": (
        ...
        "在 OpenClaw / 云养虾（ArkClaw）环境调度子代理用 `sessions_spawn` 工具：\n"
        "- 子代理名即 `.openclaw/skills/` 下的 skill 名（{names}）\n"
...
def _allowed_tools(data: dict, name: str, platform_key: str) -> list | None:
    ...
    """
    if platform_key in ("dsh", "openclaw"):
        # dsh/openclaw 的 SKILL.md 无工具白名单字段（工具用法写进正文由模型理解）
        return None
...
def _convert_standalone_skill(text: str, name: str, platform_key: str) -> str:
    ...
    suffix = {
        ...
        "openclaw": "（由 awesome-novel 自动生成的 OpenClaw skill）",
    }[platform_key]
    fm = (
        f"---\n"
        f"name: {name}\n"
