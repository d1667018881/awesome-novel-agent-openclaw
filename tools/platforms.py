...
支持平台（扩展新平台 = 往 PLATFORMS 加一行 + 处理 agents=None 语义）：
  ...
  - openclaw → .openclaw/ agents=None（agents 即 skills，OpenClaw 无项目级 agents 目录，
                          子代理即 skill，由 sessions_spawn 调度）, skills=skills, knowledge, memory
...
PLATFORMS = {
    ...
    "openclaw": Platform("openclaw", "OpenClaw", ".openclaw", agents=None,
                         skills="skills", knowledge="knowledge", memory="memory", detect_keywords=("openclaw",)),
}
...
# ---------------------------------------------------------------
# OpenClaw skill 生成（agents + skills 源 → .openclaw/skills/<name>/SKILL.md）
# ---------------------------------------------------------------

# 子代理注入段：OpenClaw 子代理默认 maxSpawnDepth=1 天然无调度权，但显式声明
# 硬约束更稳（防止未来配置放宽后递归派生），且子代理上下文只注入 AGENTS.md + TOOLS.md，
# 本段随 SKILL.md 正文被子代理 Read 读取，作为身份边界。
_OPENCLAW_DISPATCH_BAN = """## 调度权限硬约束（最高优先级，先于本文件其余一切指令）

你是执行型子代理，没有任何调度权。**禁止使用 `sessions_spawn` 工具派生子代理**
（包括派发与自己同名的 agent，禁止任何形式的递归派生）。
...
任务需要其他子代理协作（如发现需要新增设定/归档/评审）时，不要自己调度——
在回复中明确告诉 novel-agent"下一步应调度谁、为什么"，由 novel-agent 调度。"""


def _convert_to_openclaw(text: str, inline_sops=None, run_as: str = "subagent") -> str:
    """Claude Code agent frontmatter → OpenClaw skill frontmatter。

    OpenClaw 与 Claude Code 约定同源（目录 + SKILL.md），无项目级 agents 目录——
    agents 即 skills，子代理由 novel-agent 用 sessions_spawn 按 skill 名调度。
    转换要点：
    - frontmatter: 只保留 name + description（OpenClaw SKILL.md 最低要求；
      无 tools 字段——OpenClaw 不声明工具白名单，工具用法写进正文由模型理解）
    - body: agent 身份段 + 内联的专属 SOP 全文
    - novel-agent（入口调度者）追加 OpenClaw 调度适配段；其余为 subagent
      并注入调度权限硬约束
    """
    data, body = split_frontmatter(text)    # 单源解析（review #38：坏 split 已弃用）
    if not data:
    ...
    desc = str(data.get("description", "") or "").strip().replace('"', "'")
    if run_as == "subagent":
        desc = f"{desc}（awesome-novel 子代理，由 novel-agent 用 sessions_spawn 调度；先读本文件再执行 order）"
    ...
    if name == "novel-agent":
        agent_body += (
            "\n\n## OpenClaw 调度适配（本环境无 Agent 工具）\n"
            "在 OpenClaw 环境调度子代理用 `sessions_spawn` 工具：\n"
            "- 子代理名即 `.openclaw/skills/` 下的 skill 名（writer / volume-planner / "
            "chapter-planner / prompt-crafter / anti-ai / reader / updater / style-distiller）\n"
            "- task 参数第一行写 `[Subagent Task]`，内容包含：你是谁派来的（novel-agent）、"
            "先 Read 你的操作手册 `.openclaw/skills/<子代理名>/SKILL.md`、再 Read order 文件"
            "（路径写清）获取任务、完成后把 order 的 `status` 覆盖为 `DONE` 并结束\n"
            "- spawn 后调用 `sessions_yield` 结束当前轮次，等待子代理完成事件（基于推送，禁止轮询）\n"
            "- 子代理是隔离上下文，只读 order 与指定文件；order 文件协议（status: DONE）不变\n"
            "- 一次只调度一个任务，等 DONE 后再调度下一个；禁止把 novel-agent 本身作为子代理调度\n"
        )
    else:
        agent_body = _OPENCLAW_DISPATCH_BAN + "\n\n" + agent_body
    sop_sections = []
    for sop in (inline_sops or []):
        if sop and sop.exists():
            sop_sections.append(
...
def _convert_openclaw_inline_skill(text: str, name: str) -> str:
    """纯正文 SOP → OpenClaw inline skill（无 SOP 依赖的独立交互工具）。"""
    desc = ""
    for ln in text.split("\n"):
        if ln.strip().startswith("# ") and not ln.strip().startswith("## "):
            desc = ln.strip().lstrip("# ").strip()
    ...
    fm = (
        f"---\n"
        f"name: {name}\n"
        f"description: \"{desc or name}（由 awesome-novel 自动生成的 OpenClaw skill）\"\n"
        f"---\n"
    )
    return fm + "\n" + text.strip()


def deploy_openclaw_skills(project: Path, skill_home: Path, platform: Platform) -> None:
    """生成 <project>/<platform.root>/skills/<name>/SKILL.md（11 个），引用改写为平台路径。

    仅 openclaw 平台调用（agents=None）。OpenClaw 无项目级 agents 目录，agents 即 skills，
    产物与 reasonix/zcode 同构（9 个 agent + memory-recording/roleplay-sandbox 独立工具）。
    子代理 skill 不依赖 OpenClaw 的自动发现/注入——子代理被 sessions_spawn 后
    先 Read 自己的 SKILL.md 作为操作手册，因此无需 extraDirs 配置。
    """
    ...
    for agent_name, sops in exec_agents.items():
        ...
        if not agent_file.exists():
            continue
        sop_files = [skills_dir / f"{s}.md" for s in sops]
        body = _convert_to_openclaw(agent_file.read_text(encoding="utf-8"),
                                    inline_sops=sop_files, run_as="subagent")
        body = rewrite_refs(body, platform)
        skill_dir = target / agent_name
        skill_dir.mkdir(parents=True, exist_ok=True)
        (skill_dir / "SKILL.md").write_text(body, encoding="utf-8")
    ...
    novel_file = agents_dir / "novel-agent.md"
    if novel_file.exists():
        body = _convert_to_openclaw(novel_file.read_text(encoding="utf-8"),
                                    inline_sops=[skills_dir / "novel-dispatch.md"],
                                    run_as="inline")
        body = rewrite_refs(body, platform)
        skill_dir = target / "novel-agent"
    ...
    for skill_name in ("memory-recording", "roleplay-sandbox"):
        sf = skills_dir / f"{skill_name}.md"
        if sf.exists():
            body = _convert_openclaw_inline_skill(sf.read_text(encoding="utf-8"), skill_name)
            body = rewrite_refs(body, platform)
            skill_dir = target / skill_name
            skill_dir.mkdir(parents=True, exist_ok=True)
            (skill_dir / "SKILL.md").write_text(body, encoding="utf-8")
