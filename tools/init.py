        ...
        for item in SOURCE_TEMPLATES.rglob("*"):
            if item.is_file() and item.name != ".gitkeep":
                ...
                if platform.key in ("codex", "openclaw") and item.name == "CLAUDE.md":
                    # Codex 只读 AGENTS.md；OpenClaw 子代理上下文只注入 AGENTS.md + TOOLS.md
                    continue
                if item.name in ("AGENTS.codex.md", "AGENTS.openclaw.md"):
                    # 模板源仅用于生成对应平台的 AGENTS.md，不复制进项目
                    continue
                if platform.key == "codex" and item.name == "AGENTS.md":
                    ...
                    if codex_tpl.exists():
                        ...
                        continue
                if platform.key == "openclaw" and item.name == "AGENTS.md":
                    oc_tpl = SOURCE_TEMPLATES / "AGENTS.openclaw.md"
                    if oc_tpl.exists():
